function threshold = set_threshold_em(V)
    ...
    pdf_1 = @(x) weights_sorted(1)*(1 / sqrt(2 * pi * sigma_1)) * exp(-(x - mu_sorted(1)).^2 / (2 * sigma_1));
    pdf_2 = @(x)  weights_sorted(2)*(1 / sqrt(2 * pi * sigma_2)) * exp(-(x - mu_sorted(2)).^2 / (2 * sigma_2));
   ...
   test_vals = linspace(min(min(V),0),max(V),1000);
    ...
    threshold = fzero(intersection_func,[mu_sorted(1),mu_sorted(2)+2*sigma_2]);
