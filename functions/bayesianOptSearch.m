function [lambda_opt] = bayesianOptSearch(lambda_space, parameters)
...
% Define the objective function to be minimized using Bayesian Optimization
    ...
    l1_domain = optimizableVariable('l1', [10^(-3), lambda_space(1)], 'Transform', 'log'); % s
    ...
    tic;
    if parameters.Parallel.Lambda == 1
       results = bayesopt(objectiveFunc, [l1_domain, l2_domain, l3_domain], ...
                       'AcquisitionFunctionName', 'expected-improvement-plus', ...
                       'MaxObjectiveEvaluations', parameters.BayesIter, ...
...
function [bic_val, solution] = modelObjective(lambda, parameters)
    % Assuming AIC function exists and returns a structure with fields .Feval and .Solution
    BIC_result = BIC(lambda, parameters);
