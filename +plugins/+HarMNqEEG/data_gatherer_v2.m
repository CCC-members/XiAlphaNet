function [data_struct, error_msg] = data_gatherer_v2(data, SAMPLING_FREQ, cnames, data_code, reference, age, sex, country, eeg_device, keep_signal)
import functions.*
import plugins.*
...
% keep_signal   : if true, means that the data_struct will carry out the EEG signal too, in the field "data". This is optional
...
if nepochs > 1  %At least need 3 analysis windows to calculate the cross spectral matrices
    ...
    if interpolate %Interpolate the coefficients to match the spectra to the desired frequency range
        ...
        %%%%%%%%%%%%
        %Here just to guarantee that the interpolation procedure did not introduced negative values in the spectra
