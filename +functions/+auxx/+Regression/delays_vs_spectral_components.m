...
logAP = log(1e-6 + alpha_powers);          % log(Alpha Power)
logDel = log(delays0.^(-2));               % log(tau^-2)
[logDel_sorted, idxSort] = sort(logDel);
logAP_sorted = logAP(idxSort);
...
xlabel('log(\tau^{-2})', 'FontWeight', 'bold');
ylabel('log(\alpha)', 'FontWeight', 'bold');
title('Alpha Power ~ \tau^{-2} (Robust Linear & Quadratic)');
...
logXIP = log(1e-6 + xi_powers); 
logDel = log(delays0.^(-2));
[logDel_sorted, idxSort] = sort(logDel);
logXIP_sorted = logXIP(idxSort);
...
xlabel('log(\tau^{-2})', 'FontWeight', 'bold');
ylabel('log(\xi)', 'FontWeight', 'bold');
title('Xi Power ~ \tau^{-2} (Robust Linear & Quadratic)');
